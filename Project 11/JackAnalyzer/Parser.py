...
class Parser(object):
    ...
    def compile_class_name(self):
        # Class names don't have to go into the symbol table
        self._cur_class = self.compile_var_name()
    ...
    def compile_subroutine_name(self):
        # subroutine names don't have to go in the symbol table
    ...
    def write_func_decl(self, kwd):
        self.vm.write_function(self.vm_function_name(),
                               self.symbols.var_count(SK_VAR))
    ...
    def load_this_ptr(self, kwd):
        if kwd == KW_METHOD:
            ...
            self.vm.push_const(self.symbols.var_count(
                SK_FIELD))    # object size
    ...
    def compile_base_plus_index(self, name):
        ...
        self._require(T_SYM, ']')
        # base+index - leave on the stack for later
    ...
    def compile_if(self):
        ...
        # VM code for condition and if statements
    ...
    def compile_while(self):
        ...
        # VM code for condition and while statements
    ...
    def compile_term(self):
        if self._is_const():
            ...
            if self._is_sym('['):
                # VM code for array subscript
                self.compile_array_subscript(name)
            elif self._is_sym('(.'):
                # VM code for subroutine call
    ...
    def compile_const(self):
        ...
        if tok == T_NUM:
            ...
            # initialize string & push str addr
            self.write_string_const_init(val)
        elif tok == T_KEYWORD:
            # push TRUE, FALSE, NULL etc.
