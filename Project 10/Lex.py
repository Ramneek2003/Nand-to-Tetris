...
class Lex(object):
    ...
    def _writexml(self):
        ...
        if tok == T_KEYWORD:
            self._outfile.write(self.keyword())
        elif tok == T_SYM:
            self._outfile.write(escape(self.symbol()))
        elif tok == T_NUM:
            self._outfile.write(self.int_val())
        elif tok == T_STR:
            self._outfile.write(self.string_val())
        elif tok == T_ID:
            self._outfile.write(self.identifier())
        elif tok == T_ERROR:
            self._outfile.write('<<ERROR>>')
    ...
    _str_re = r'"[^"\n]*"'
    _id_re = r'[\w\-]+'
    _word = re.compile(_keyword_re+'|'+_sym_re+'|' +
                       _num_re+'|'+_str_re+'|'+_id_re)
